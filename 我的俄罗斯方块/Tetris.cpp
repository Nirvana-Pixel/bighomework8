...
Tetris::Tetris(int rows, int cols, int left, int top, int blocksize)
{
	...
	this->topMargin = top;
	this->blockSize = blocksize;	// 发现问题了；形参 blocksize 根本没有用到，是你拼写错误的问题
...
void Tetris::updateWindow()
{
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			...
			// 最左侧我打好断点，让程序在绘制已固化方块时停下来；看到没有，你这里blockSize = 0；
...
void Tetris::drawScore()
{
	...
	setbkmode(TRANSPARENT);//字体背景设置为透明效果
